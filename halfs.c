...
char serverMessage[MAX]; char c1ientMessage[MAX];
...
printf("\ntext message here .. :"); scanf("%s", serverMessage);
send(clientSocketDescriptor, serverMessage, sizeof(serverMessage) , 0); 
recv(clientSocketDescriptor, &c1ientMessage, sizeof(c1ientMessage), 0) ; printf("\nCLIENT: %s", c1ientMessage);
}
